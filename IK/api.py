...
#
# Edited by Max Kragballe Nielsen (max@di.ku.dk)
#
...
def get_joint_angles(skeleton):
    angles = torch.zeros((len(skeleton.bones)*3, ), dtype=torch.float)
    for bone in skeleton.bones:
...
def compute_jacobian(chains, skeleton):
    ...
    J = torch.zeros((num_coords, num_angles), dtype=torch.float)
    row_offset = 0
    for chain in chains:
    ...
    for chain in chains:
        ...
        for k in chain.bones:
            for h in chain.bones:
                ...
                ua = torch.dot(V3.cross(u, J_a), r)
                va = torch.dot(V3.cross(v, J_a), r)
                wa = torch.dot(V3.cross(w, J_a), r)

                ub = torch.dot(V3.cross(u, J_b), r)
                vb = torch.dot(V3.cross(v, J_b), r)
                wb = torch.dot(V3.cross(w, J_b), r)

                uc = torch.dot(V3.cross(u, J_c), r)
                vc = torch.dot(V3.cross(v, J_c), r)
                wc = torch.dot(V3.cross(w, J_c), r)
                ...
                if h != k:
                    H[k_offset:k_offset+3, h_offset:h_offset+3] -= dH.T
...
def compute_gradient(chains, skeleton, J):
    r = torch.zeros((len(chains)*3, ), dtype=torch.float)
    row_offset = 0
    for chain in chains:
        e = get_end_effector(chain, skeleton)
        r[row_offset:row_offset+3] = chain.goal - e
        row_offset += 3
    g = - torch.mm(J.T, r.view(r.shape[0], 1))
    return g
...
def finite_difference_gradient(chains, skeleton, h=0.1):
    ...
    g = torch.zeros((N, ), dtype=torch.float)
    for i in range(N):
